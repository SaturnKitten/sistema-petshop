...
exports.getAllFuncionarios = async (req, res) => {
  try {
    const result = await pool.query("SELECT * FROM Funcionario WHERE Removido = FALSE ORDER BY Nome");
...
exports.getFuncionarioById = async (req, res) => {
  try {
    const result = await pool.query("SELECT * FROM Funcionario WHERE ID = $1 AND Removido = FALSE", [req.params.id]);
...
exports.deleteFuncionario = async (req, res) => {
  try {
    const result = await pool.query(
      "UPDATE Funcionario SET Removido = TRUE WHERE ID = $1 RETURNING *",
      [req.params.id]
    );
