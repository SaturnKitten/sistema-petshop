...
exports.updateRecebimento = async (req, res) => {
  ...
  try {
    ...
    if (rows.length === 0) return res.status(404).json({ success: false, message: "Recebimento não encontrado" });
    res.json({ success: true, message: "Recebimento atualizado com sucesso", data: rows[0] });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
exports.deleteRecebimento = async (req, res) => {
  try {
    const { rows } = await db.query("UPDATE Recebimento SET Removido = TRUE WHERE ID = $1 RETURNING *", [req.params.id]);
    if (rows.length === 0) return res.status(404).json({ success: false, message: "Recebimento não encontrado" });
    res.json({ success: true, message: "Recebimento removido com sucesso", data: rows[0] });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
